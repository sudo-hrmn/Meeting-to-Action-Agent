...
**Possible follow-up:** What's the SLA you'd commit to for a production version of this?
...
**A8.** Fair challenge — let me be specific about what's non-trivial. First, structured extraction with schema validation and graceful degradation: getting an LLM to reliably return typed, validated JSON for four different extraction tasks requires careful prompt engineering and defensive parsing — this fails often without that engineering. Second, the RAG pipeline: connecting ChromaDB, managing document lifecycle, chunking strategy, and source attribution is a real engineering task. Third, the tool guardrail registry: the allowlist pattern, input validation chain, and idempotent tool design show production-thinking. Fourth, 37 tests with proper LLM mocking: testing AI systems without hitting real APIs is a skill. And fifth, the architecture itself: proper layering, async throughout, lifespan management, structured logging — these are habits, not accidents.
